class Signal {
  ...
  map(f, newType = null, newMin = null, newMax = null) {
    return new Signal(this.rank == 2 ? this.data.map(x => x.map(f)) : this.data.map(f), this.sampleRate, newType ? newType : this.type, newMin !== null ? newMin : this.min, newMax !== null ? newMax : this.max);
  }
